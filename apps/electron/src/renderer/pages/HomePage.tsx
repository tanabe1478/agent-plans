...
export function HomePage() {
  ...
  // Disable selection mode in search mode
  useEffect(() => {
    if (isSearchMode && selectionMode) {
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="min-w-0 flex-1">
            <SearchBar
              ...
              placeholder="Search plans... (e.g. status:in_progress)"
            />
          </div>
          <QueryGuidePopover onApplyQuery={handleApplyQuery} />
          <SavedSearches currentQuery={submittedQuery} onApplyQuery={handleApplyQuery} />
        </div>
        {isSearchMode && searchData && (
          <p className="mt-2 text-[11px] text-slate-500">
            {searchData.total} {searchData.total === 1 ? 'result' : 'results'} for &ldquo;
