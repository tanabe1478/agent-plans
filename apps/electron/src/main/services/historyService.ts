...
export class HistoryService {
  ...
  private get historyDir(): string {
    return join(this.plansDir, '.history');
  }

  /**
   * Ensure the history directory for a plan exists
   */
  private async ensureHistoryDir(filename: string): Promise<string> {
    const dir = join(this.historyDir, filename);
   ...
   */
  async saveVersion(filename: string, content: string, summary: string): Promise<PlanVersion> {
    const dir = await this.ensureHistoryDir(filename);
    ...
    await this.pruneVersions(filename);
   ...
   */
  async listVersions(filename: string): Promise<PlanVersion[]> {
    const dir = join(this.historyDir, filename);
   ...
   */
  async getVersion(filename: string, version: string): Promise<string> {
    const dir = join(this.historyDir, filename);
   ...
   */
  async rollback(filename: string, version: string): Promise<void> {
    ...
    await this.saveVersion(filename, currentContent, 'Before rollback');

    // Get the version content and write it as current
    const versionContent = await this.getVersion(filename, version);
   ...
   */
  private async pruneVersions(filename: string): Promise<void> {
    const dir = join(this.historyDir, filename);
...
const defaultHistoryService = new HistoryService({ plansDir: config.plansDir });
...
export async function saveVersion(
  ...
  return defaultHistoryService.saveVersion(filename, content, summary);
}

export async function listVersions(filename: string): Promise<PlanVersion[]> {
  return defaultHistoryService.listVersions(filename);
}

export async function getVersion(filename: string, version: string): Promise<string> {
  return defaultHistoryService.getVersion(filename, version);
}

export async function rollback(filename: string, version: string): Promise<void> {
  return defaultHistoryService.rollback(filename, version);
}
